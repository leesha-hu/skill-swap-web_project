...
const getSkillClasses = async (req, res) => {
    ...
    try {
        ...
        const [classes] = await db.query('select s.name,c.start_time,c.end_time,c.date from class_timings c join skills s on s.skill_id=c.skill_id where s.skill_id=?', [skillId])
        if (classes.length === 0) {
